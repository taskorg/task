	...
	var uc_account_status = {'INVALID':'无效','SEND':'已发放'};
	var wealtype = {'EXCHARGE':'充值','TASK':'任务'};
	var load_option = {
			p:1,
			per:1,
			page_id:'page',
			url : J_task.Config.website+"ucenter/scorelog.action",
			params:{},
			showlist:function(result){
				...
				$.each(result,function(k,v){
					...
					$tr.append("<td height='36' align='center'>"+new Date(v.audit_time).format("yyyy-MM-dd hh:mm:ss")+"</td>");
					...
					$tr.append("<td class='color_hs' align='center'>"+v.wealth_balance+"</td>");
					$tr.append("<td align='center'>"+wealtype[v.wealth_type]+"</td>");
					$tr.append("<td align='center'>"+v.remark+"</td>");
					$tr.append("<td align='center'>"+uc_account_status[v.status]+"</td>");
					$tr.hide();
	...
	$.ajax({
		url : J_task.Config.website+"ucenter/scorelogcount.action",
		type : "POST",
		dataType : "JSON",
		jsonp : "jsoncallback",
		async:false,
